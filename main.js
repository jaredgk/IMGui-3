...
fs.open('imgui-backend.log','w', function(err) {
...
var jobIdx = 0;
var jobList = [];
var IMA_PATH_UNIX = path.join(dirPath,'IMa','IMa3_xml');
var IMA_PATH_WIN = path.join('resources','app','IMa','IMa2.exe');
var IMFIG_PATH_UNIX = path.join(dirPath,'scripts','IMfig.py');
var IMFIG_PATH_MAC = path.join(dirPath,'scripts','IMfig3-mac').replace(/[ ]/g,'%20');
var IMFIG_PATH_WIN = path.join(dirPath,'scripts','IMfig.exe').replace(/[ ]/g,'%20');
var PATHTEST_PATH_UNIX = path.join(dirPath,'scripts','testpath.sh').replace(/[ ]/g,'%20');
var PATHTEST_PATH_WIN = path.join(dirPath,'scripts','testpath.bat').replace(/[ ]/g,'%20');
var PORT_NUMBER = 3000;
var DEBUG_FLAG = 0;

//Prints debug message if DEBUG_FLAG is less than priority.
//If DEBUG_FLAG = 0, all messages will be printed
function cLog(msg,priority = 0) {
    if(priority >= DEBUG_FLAG) {
        console.log(msg);
        msgs = JSON.stringify(msg) + '\n';
        //console.log(JSON.stringify(msg instanceof Buffer));
        if (msg instanceof Buffer) {
            fs.appendFileSync('imgui-backend.log',msg);
        } else {
            fs.appendFileSync('imgui-backend.log',msgs);
        }
    }
}
...
function parseFigArgs(args) {
    var spawn_args = [];
    spawn_args.push('python');
    var cmd_args = [];
    if(os.platform() === 'linux') { cmd_args.push(IMFIG_PATH_UNIX); }
    else if(os.platform() === 'win32') { cmd_args.push(IMFIG_PATH_WIN); }
    else if(os.platform() === 'darwin') { cmd_args.push(IMFIG_PATH_MAC); }
    cLog(args,1);
...
function getCaption(filename) {
    ...
    var parasep = os.EOL+os.EOL;
    var paragraphs = contents.split(parasep);
    return paragraphs[1];
}
...
ipcMain.on('remove',function(e,data) {
    ...
    if(jobList[id].status !== 0) {
        ...
        cLog('Warning: job removal of job with status 0');
    }
});
