...
//Data in XML format
function addHistogramObject(x) {
    var obj = {};
    obj.type = 'histo';
    obj.description = x.getAttribute('title');
    for(var i = 0; i < x.childNodes.length; i++) {
        if(x.childNodes[i].nodeName === "Variables") {
    ...
    var num = xmlArray.length
    var opt = '<option value="'.concat(num,'">',obj.description,'</option>');
    xmlArray.push(obj);
...
function displaySelected(x) {
    ...
    if (x.type === 'table') {
        ...
        $('#savefiletable').show();
        createTable(x);
    } else if (x.type === 'histo') {
        $('#selectid').show();
        $('#svgdiv').show();
        $('#chartdiv').show();
        createPullTab(x.parameters);
    } else if (x.type === 'treetop') {
        $('#selectid').show();
        $('#tablediv').show();
        $('#savefiletable').show();
        createTreeTop(x);
...
function createTreeTop(xmlobj) {
    ...
    $('#outtable').append(trr);
    var poplist = $.trim(xmlobj.pops).split(/[ ][ ]*/);
    var thr = '<tr><th>Number</th><th>Population</th></tr>';
    for (i = 0; i < poplist.length; i++) {
        var el = '<tr><td>'.concat(i,'</td><td>',poplist[i],'</td></tr>');
        thr += el;
    }
    $('#poptable').append(thr);
}
...
function xLog(xn,num) {
    console.log(num);
    console.log((new XMLSerializer()).serializeToString(xn));
}


function addTable(xmlobj) {
    var obj = {};
    obj.type = 'table';
    obj.description = xmlobj.childNodes[0].nodeValue;
    obj.sub = '';
    for (var i=0; i < xmlobj.childNodes.length; i++) {
        if (xmlobj.childNodes[i].nodeName === "Head" ) {
            obj.head = xmlobj.childNodes[i].childNodes[0].nodeValue;
        } else if (xmlobj.childNodes[i].nodeName === "Subhead") {
            obj.sub = xmlobj.childNodes[i].childNodes[0].nodeValue;
        } else if (xmlobj.childNodes[i].nodeName === "Data") {
            obj.data = xmlobj.childNodes[i].childNodes;
        }
    }
    var num = xmlArray.length;
    var opt = '<option value="'.concat(num,'">',obj.description,'</option>');
...
function addTreeTop(xmlobj) {
    ...
    for (var i = 0; i < xmlobj.childNodes.length; i++) {
        if (xmlobj.childNodes[i].nodeName === "Head") {
            obj.head = xmlobj.childNodes[i].childNodes[0].nodeValue;
        } else if (xmlobj.childNodes[i].nodeName === "Populations") {
            obj.pops = xmlobj.childNodes[i].childNodes[0].nodeValue;
        } else if (xmlobj.childNodes[i].nodeName === "Data") {
            obj.data = xmlobj.childNodes[i].childNodes;
        }
    }
    var num = xmlArray.length;
    var opt = '<option value="'.concat(num,'">Tree Topology</option>');
    xmlArray.push(obj);
    $(opt).appendTo('#selectid');
}
...
function loadMain() {
    ...
    for (i = 0; i < x.length; i++) {
        //xLog(x[i],2);
        if (x[i].nodeName === "AcceptanceRates") {
            //console.log(x[i].childNodes.length);
            for (ii = 0; ii < x[i].childNodes.length; ii++) {
                if(x[i].childNodes[ii].nodeName === 'UpdateRates') {
                    addTable(x[i].childNodes[ii]);
                }
            }
        } else if(x[i].nodeName === "AllHistograms") {
            for (ii = 0; ii < x[i].childNodes.length; ii++) {
                if(x[i].childNodes[ii].nodeName === "Histogram") {
                    addHistogramObject(x[i].childNodes[ii]);
                }
            }
        } else if(x[i].nodeName === "TreeTopology") {
            addTreeTop(x[i]);
        }
    }
    displaySelected(xmlArray[0]);
}
...
$('#xmlsb').click(function () {
    ...
    if(files.length === 1) {
        ...
        if(xmln.indexOf('.intervals.xml') > -1) {
            xmlFile = 'intervals';
