...
function printParams() {
    var c = '-c'; var j = '-j'; var p = '-p'; var r = '-r'; var cmdline = ''; var f = '-f'; var b = '-b'; var d = '-d'; var g = '-g'; var hn = '-hn'; var ha = '-ha'; var hb = '-hb'; var i = '-i'; var l = '-l'; var m = '-m'; var o = '-o'; var q = '-q'; var s = '-s'; var t = '-t'; var u = '-u'; var v = '-v'; var w = '-w'; var x = ''; var y = '-y'; var z = '-z'; var radios = document.getElementById('heating-type'); var ii;
    ...
    if (document.getElementById('c0').checked) { c += '0'; }
    if (document.getElementById('c1').checked) { c += '1'; }
    if (document.getElementById('c2').checked && ($('input[name="run-mode"]:checked').val() === 'l')) {
        c += '2';
        w += getName('#nestedf');
    }
    ...
    if (document.getElementById('c4').checked) { c += '4'; }
    ...
    if (document.getElementById('j0').checked) { j += '0'; }
    ...
    if (document.getElementById('jx').checked) { j += 'X'; }
    if (document.getElementById('p0').checked) { p += '0'; }
    if (document.getElementById('p1').checked) { p += '1'; }
    if (document.getElementById('p2').checked && ($('input[name="run-mode"]:checked').val() === 'm')) { p += '2'; }
    ...
    if (document.getElementById('p4').checked) { p += '4'; }
    if (document.getElementById('p5').checked) { p += '5'; }
    if (document.getElementById('p6').checked) { p += '6'; }
    if (document.getElementById('p7').checked && ($('input[name="run-mode"]:checked').val() === 'm')) { p += '7'; }
    if (document.getElementById('r0').checked && ($('input[name="run-mode"]:checked').val() === 'l')) {
        r += '0';
        v += getName('#titag');
    }
    if (document.getElementById('r1').checked) { r += '1'; }
    if (document.getElementById('r2').checked && ($('input[name="run-mode"]:checked').val() === 'm')) { r += '2'; }
    ...
    if (document.getElementById('r6').checked) { r += '6'; }
    if (document.getElementById('r7').checked) { r += '7'; }
    if (blstat('#burn') === 2 || blstat('#run-time') === 2) { r += '8'; }
    if (document.getElementById('phylo-input').value) {
        var x_list = document.getElementById('phylo-input').value.toString().trim().split(";");
        for (x_arg in x_list) {
            //x_nums = x_arg.split(/[ \t]+/);
            x += "-x " + x_list[x_arg].trim() + " ";
...
function validatePaths() {
    ...
    if($('#c3').is(':checked')) { sendObj.paramf = getName('#paramf'); }
    if($('#r0').is(':checked')) { sendObj.titag = getName('#titag'); }
    if($('#r3').is(':checked')) { sendObj.mcftag = getName('#mcftag'); }
    if($('#c2').is('checked')) { sendObj.nestedf = getName('#nestedf'); }
...
function validateArgs() {
    ...
    var prior_check = $('#c3').is(':checked') ? 0 : 1;
    $('#error-field').hide();
    $('.err-add').remove();
    $('.btn-danger').removeClass('btn-danger');
    $('.bg-danger').removeClass('bg-danger');
    $('#valid').hide();
    if (v_check_line(document.getElementById('c2'), document.getElementById('nestedf').value) !== 1) {
        fl = 1;
        errmsg('Option "c2" requires filename for nested models');
        $('#c2_text').addClass('bg-danger');
        $('#b2').addClass('btn-danger');
    }
    if (v_check_line(document.getElementById('c3'), document.getElementById('paramf').value) !== 1) {
        fl = 1;
        alert('Option "c3" requires base name for parameter prior files');
        $('#c3_text').addClass('bg-danger');
        $('#b2').addClass('btn-danger');
    }
    if (v_check_line(document.getElementById('r0'), document.getElementById('titag').value) !== 1) {
        fl = 1;
        errmsg('Option "r0" requires base name for *ti files');
        $('#r0_text').addClass('bg-danger');
        $('#b5').addClass('btn-danger');
    }
    if (v_check_line(document.getElementById('r3'), document.getElementById('mcftag').value) !== 1) {
        fl = 1;
        errmsg('Option "r3" requires base name for *mcf files');
        $('#r3_text').addClass('bg-danger');
        $('#b5').addClass('btn-danger');
    }
    ...
    if ($('#c4').is(':checked')) {
        if ($('#burn-noc').val() < 100 || ($('#burn-first').val().length > 0 && $('#burn-second').val().length === 0)) {
            fl = 1;
            errmsh('Option "c4" requires chain count over 100 and to use both of -ha and -hb or neither of them');
            $('#c4').addClass('btn-danger');
        }
    }
    ...
    if ($('#phylo-input').val().length !== 0) {
        var x_list = document.getElementById('phylo-input').value.toString().trim().split(";");
        for (x_arg in x_list) {
            x_set = x_list[x_arg].trim().split(/[ \t]+/);
            if (x_set.length === 0) { continue; }
            if (x_set.length !== 3) {
                errmsg('X argument ' + x_set + ' does not have three elements');

            }
            for (i = 0; i < 3; i++) {
                if (isNaN(parseFloat(x_set[i]))) {
                    errmsg('Value ' + i + ' of X argument ' + x_arg + ' is not a number');
                }
            }
        }
    }
